...
author: "Magnus Krogh"
date: " created 4 October 2021, updated 29 November 2022"
output:
  html_document:
    toc: true
    toc_float: true
...
```{r relabel column}
names(Western_Europe) <- c("entity", "code", "year", "homocides_per_100k")
```
...
ggplot(data = Western_Europe) + 
  geom_col(aes(x = year, y = homocides_per_100k, color = entity)) + 
  labs(title = "Homicides in Europe", x = "Years", y = "Homicides per 100k")
...
The duration of the kings' reign increases over time and it is quite comparable to the number of homicides which decreases over time. 

```{r eval = TRUE}
Kings_2 <- read.csv2("data/kings_1.csv")
Kings_2 %>% 
...
By looking at the visualization of the number of homicides through time, it clearly appears that the number of homicides were a lot higher back in the days and has been decreasing in number since year 1300. The amount of homicides in Western European countries in the 21st century is very low, and this could definitely indicate that we are a lot more civilized in our day and age. When we look at the duration of reigns in the dataset about Danish monarchs we see the same trend. As we go forward in time the duration of reign increases drastically which is very convenient to our hypothesis as we also see an overall decrease in homicide during the same period. However al lot of other explanations could be present, for instance a basic improvement in living standards and medical sciences. 
